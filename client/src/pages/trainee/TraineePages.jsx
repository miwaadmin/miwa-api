...
// Reduce a markdown brief down to one readable paragraph for the dashboard.
// Strips headings, bullets, links, and trims to ~280 chars on a clean break.
function summarizeBrief(markdown) {
  ...
  const stripped = markdown
    ...
    .map(line => line.replace(/^[-*+>\d.]+\s+/, '').replace(/[*_`]/g, '').replace(/\[([^\]]+)\]\([^)]+\)/g, '$1'))
    .join(' ')
    .replace(/\s+/g, ' ')
    .trim()
      ...
      <section className="rounded-2xl border border-brand-100 bg-white p-5 shadow-sm">
        <div className="flex items-center justify-between gap-3 mb-3">
          <div>
            ...
            <p className="text-xs text-gray-500">A one-paragraph pulse across cases, drafts, supervision, and hours.</p>
          </div>
          <button
            type="button"
            onClick={() => {
        ...
        {briefLoading ? (
          ...
          <>
            <p className="text-sm text-gray-700 leading-relaxed">{summarizeBrief(brief)}</p>
            <div className="mt-3 flex justify-end">
              <Link
                to="/t/learning#research-briefs"
...
export function TraineeLearning() {
  ...
  useEffect(() => {
    ...
    apiFetch('/research/briefs')
      .then(r => r.ok ? r.json() : [])
      .then(data => { if (!cancelled) setBriefs(Array.isArray(data) ? data : []) })
      .catch(() => {})
      .finally(() => { if (!cancelled) setBriefsLoading(false) })
    return () => { cancelled = true }
  }, [])
  // Auto-scroll to the Research Briefs section when arriving via #research-briefs
  useEffect(() => {
    if (typeof window === 'undefined') return
    if (window.location.hash !== '#research-briefs') return
    const el = document.getElementById('research-briefs')
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-5">
      ...
      <section id="research-briefs" className="rounded-2xl border border-gray-200 bg-white p-5 scroll-mt-6">
        <div className="flex items-center justify-between gap-3 mb-3">
          <div>
            <h2 className="text-sm font-bold text-gray-950">Research briefs</h2>
            <p className="mt-1 text-xs text-gray-500">Daily, weekly, and crisis briefings on what's emerging in the field — generated by Miwa.</p>
          </div>
          <Link to="/briefs" className="text-xs font-bold text-brand-600 hover:text-brand-700">Open Briefs</Link>
        </div>
        {briefsLoading ? (
          <p className="text-sm text-gray-500">Loading briefs…</p>
        ) : briefs.length === 0 ? (
          <p className="text-sm text-gray-500">No briefs yet. Generate one from the Briefs page.</p>
        ) : (
          <div className="space-y-2">
            {briefs.slice(0, 6).map(brief => {
              const isOpen = expandedBriefId === brief.id
              return (
                <div key={brief.id} className="rounded-xl border border-gray-200 bg-white">
                  <button
                    type="button"
                    onClick={() => setExpandedBriefId(isOpen ? null : brief.id)}
                    className="w-full px-4 py-3 text-left flex items-start justify-between gap-3 hover:bg-gray-50 rounded-xl"
                  >
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-bold uppercase tracking-wide text-brand-600">{brief.type || 'brief'}</span>
                        {brief.local_date && (
                          <span className="text-[11px] text-gray-400">· {brief.local_date}</span>
                        )}
                      </div>
                      <p className="mt-1 text-sm font-semibold text-gray-900 leading-snug">{brief.title || 'Untitled brief'}</p>
                      {!isOpen && brief.content && (
                        <p className="mt-1 text-xs text-gray-500 line-clamp-2">{summarizeBrief(brief.content)}</p>
                      )}
                    </div>
                  ...
                  {isOpen && brief.content && (
                    <div className="px-4 pb-4 prose-clinical text-sm" dangerouslySetInnerHTML={{ __html: renderClinical(brief.content) }} />
                  )}
                </div>
              )
            })}
